...
class Flashpoint(Magics):
    ...
    def __init__(self, shell, debug=False, *args, **kwargs):
        ...
        if self.name_str in self.shell.user_ns['jupyter_loaded_integrations']:
            jiu.displayMD(f"**[ ! ]** Potenital Multiverse collision of names: {self.name_str}")
            jiu.displayMD(f"**[ * ]** {self.shell.user_ns['jupyter_loaded_integrations']}")
        else:
    ...
    def flashpoint(self, line, cell=None):
        if not self.name_str in self.shell.user_ns['jupyter_loaded_integrations']:
            ...
            if self.shell.user_ns['jupyter_loaded_integrations'][self.name_str] != f"{self.name_str}_base":
                jiu.displayMD(f"**[ ! ]** We should only get here with a {self.name_str}_base state. Currently for {self.name_str}: {self.shell.user_ns['jupyter_loaded_integrations'][self.name_str]}")
            else:
                if self.debug:
